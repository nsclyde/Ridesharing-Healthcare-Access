---
title: "Empirical Analyses Reproduction for The Impact of Ridesharing Platforms on Healthcare Access"
author: "Nick Clyde"
date: "2023-09-26"
output:
  html_document:
...
census_df = read.csv("Data/city_census_data.csv")

uber_providers <- merge(uber_providers, census_df, by.y = c("year", "city"), by.x = c("yearindicator", "Prscrbr_City"),
  all.x = TRUE)
```
...
uber_providers$claimsperbene=ifelse((is.na(uber_providers$Tot_Benes)==FALSE),uber_providers$Tot_Clms/uber_providers$Tot_Benes,NA)
...
temp2=temp[,c("Tot_Clms","Tot_Benes","claimsperbene","Bene_Avg_Risk_Scre","avg_pre_dual","standardized_pop","standardized_elderlypop","standardized_medianincome","Prscrbr_City","yearindicator")]

#First Austin in before periods
summary_stats=as.data.frame(describe(subset(temp2, Prscrbr_City=="Austin" & yearindicator<2018)))
selected_stats <- summary_stats[, c("n", "mean", "sd")]
...
# create latex, but need to remove last two rows that aren't required
...
```{r}
filtered_data=subset(uber_providers, similar_to_austin==1)
claims_cutoff <- filtered_data[filtered_data$Tot_Clms <= quantile(filtered_data$Tot_Clms, 0.75), ]

filtered_data=subset(filtered_data,is.na(Tot_Benes)==FALSE)
benes_cutoff <- filtered_data[filtered_data$Tot_Clms <= quantile(filtered_data$Tot_Benes, 0.95), ]

hist(claims_cutoff$Tot_Clms, breaks=50, main="Total Claims Histogram", xlab="Number of Total Claims", ylab="Number of Providers", col="blue", border="black",cex.main=1.5, cex.lab=1.2, cex.axis=1)

#hist(data, main="Histogram Title", xlab="X-axis Label", ylab="Y-axis Label", cex.main=1.5, cex.lab=1.2, cex.axis=1)

hist(benes_cutoff$Tot_Benes, breaks=50, main="Total Beneficiaries Histogram", xlab="Number of Total Beneficiaries", ylab="Number of Providers", col="blue", border="black",cex.main=1.5, cex.lab=1.2, cex.axis=1)
```
...
paralleltrends = felm(Tot_Clms ~ I(treatment_group*yearindicator) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome| PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers, similar_to_austin==1 & yearindicator<2018))

paralleltrends3 = felm(Tot_Benes ~ I(treatment_group*yearindicator) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome| PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers, similar_to_austin==1 & yearindicator<2018))
...
column3 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1))
...
stargazer(column1,column2,column3, digits=2, no.space=TRUE, p.auto=TRUE, omit.stat=c("ser", "adj.rsq","rsq"),
          add.lines = list(
                           ...
                           c("Other controls", 'No', 'No', 'Yes')),type="text")
...
column3 = felm(Tot_Benes ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1))
...
stargazer(column1,  column2, column3, digits=4, report = "vcsp", no.space=TRUE, p.auto=TRUE, omit.stat=c("ser", "adj.rsq","rsq"),
          add.lines = list(
                           c("Year fixed effects", 'No', 'Yes', 'Yes'), 
                           c('Provider fixed effects', 'Yes','Yes','Yes'),
                           c("Other controls", 'No', 'No', 'Yes')),type="text")
...
column3 = felm(Tot_Clms ~ I(treatment_group*I(yearindicator==2013)) + I(treatment_group*I(yearindicator==2014)) + I(treatment_group*I(yearindicator==2015))+ I(treatment_group*I(yearindicator==2016)) + I(treatment_group*I(yearindicator==2018)) + I(treatment_group*I(yearindicator==2019)) + I(treatment_group*I(yearindicator==2020)) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1))

stargazer(column3, digits=2, no.space=TRUE, p.auto=TRUE, omit.stat=c("ser", "adj.rsq","rsq"),
          add.lines = list(
                           c("Year fixed effects", 'No', 'Yes', 'Yes'), 
                           c('Provider fixed effects', 'Yes','Yes','Yes'),
                           c("Other controls", 'No', 'No', 'Yes')),type="text")
...
# Create dataframe
time_effect_estimates <- data.frame(
  year = factor(c(2013, 2014, 2015, 2016, 2018, 2019, 2020)),
  estimate = c(-43.39, -21.58, -33.04, -11.61, -73.43, -194.81, -179.83),
  se = c(83.61, 62.26, 42.93, 23.17, 26.61, 89.35, 53.78)
)

time_effect_estimates$lower <- time_effect_estimates$estimate - 1.96 * time_effect_estimates$se
time_effect_estimates$upper <- time_effect_estimates$estimate + 1.96 * time_effect_estimates$se

# Plot with error bars
ggplot(time_effect_estimates, aes(x = year, y = estimate)) +
  geom_point(size = 3, color = "blue") + 
  geom_errorbar(aes(ymin = lower, ymax = upper), width = 0.2) +
  geom_hline(yintercept = 0, linetype = "dashed", color = "red") + 
  labs(
       x = "Year",
       y = "Austin*Year Coefficient Estimate") +
  theme_minimal()
...
paralleltrends = felm(I(Tot_Clms-Opioid_Tot_Clms) ~ I(treatment_group*yearindicator) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome| PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers, similar_to_austin==1 & yearindicator<2018))
...
column1 = felm(I(Tot_Clms-Opioid_Tot_Clms) ~ I(treatment_group*AfterReentry) + AfterReentry |PRSCRBR_NPI|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1))

column2 = felm(I(Tot_Clms-Opioid_Tot_Clms) ~ I(treatment_group*AfterReentry) |PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1))


column3 = felm(I(Tot_Clms-Opioid_Tot_Clms) ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1))

#column4 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome + standardized_white + standardized_hispanic|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1))
...
stargazer(column3, digits=2, no.space=TRUE, p.auto=TRUE, omit.stat=c("ser", "adj.rsq","rsq"),
          add.lines = list(
                           ...
                           c('Provider fixed effects', 'Yes','Yes','Yes'),
                           c("Other controls", 'Yes', 'No', 'Yes')),type="text")

temp=subset(uber_providers,similar_to_austin==1 & treatment_group==0 & is.na(Opioid_Tot_Clms)==FALSE)
temp$difference=temp$Tot_Clms-temp$Opioid_Tot_Clms
...
stand_dev=sd(subset(hte,similar_to_austin==1 & is.na(avg_pre_risk)==FALSE)$avg_pre_risk)

hte$st_avg_pre_risk=hte$avg_pre_risk/stand_dev
```
...
hte$AfterReentry=ifelse(hte$yearindicator>2017,1,0)

column1 = felm(Tot_Clms ~  I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(hte,similar_to_austin==1 & is.na(st_avg_pre_risk)==FALSE))

column2 = felm(Tot_Clms ~  I(treatment_group*AfterReentry)+ I(I(treatment_group*AfterReentry)*st_avg_pre_risk) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(hte,similar_to_austin==1 & is.na(st_avg_pre_risk)==FALSE))


column3 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(hte,similar_to_austin==1 & is.na(avg_pre_dual)==FALSE))

column4 = felm(Tot_Clms ~  I(treatment_group*AfterReentry) + I(I(treatment_group*AfterReentry)*avg_pre_dual) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(hte,similar_to_austin==1 & is.na(avg_pre_dual)==FALSE))
...
stargazer(column1,column2, column3, column4, digits=2, no.space=TRUE, p.auto=TRUE, single.row=FALSE, omit.stat=c("ser", "adj.rsq","rsq"),
          add.lines = list(c("Relative effect size", 'Yes', 'Yes', 'Yes','Yes', 'Yes','Yes'), 
                           c("Year fixed effects", 'Yes', 'Yes', 'Yes','Yes', 'Yes','Yes'), 
                           c('Provider fixed effects', 'Yes','Yes','Yes','Yes', 'Yes','Yes'),
                           c('Other controls', 'Yes','Yes','Yes','Yes', 'Yes','Yes')), type="text")


mean(subset(hte,similar_to_austin==1 & treatment_group==0 & is.na(st_avg_pre_risk)==FALSE)$Tot_Clms)
mean(subset(hte,similar_to_austin==1 & treatment_group==0 & is.na(avg_pre_dual)==FALSE)$Tot_Clms)
...
column1 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & is.na(st_risk_2015)==FALSE))

column2 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + I(I(treatment_group*AfterReentry)*st_risk_2015) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & is.na(st_risk_2015)==FALSE))


column3 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & is.na(dual_2015)==FALSE))

column4 = felm(Tot_Clms ~I(treatment_group*AfterReentry) + I(I(treatment_group*AfterReentry)*dual_2015) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & is.na(dual_2015)==FALSE))
...
mean(subset(uber_providers,similar_to_austin==1 & treatment_group==0 & is.na(st_risk_2015)==FALSE)$Tot_Clms)
mean(subset(uber_providers,similar_to_austin==1 & treatment_group==0 & is.na(dual_2015)==FALSE)$Tot_Clms)
...
provider_numbers <- uber_providers %>%
  group_by(Prscrbr_City, yearindicator) %>%
  summarise(value = n_distinct(PRSCRBR_NPI))


provider_numbers$City=provider_numbers$Prscrbr_City
provider_numbers$t=provider_numbers$yearindicator-2013



provider_numbers$X=0
provider_numbers$X=ifelse(provider_numbers$City=="Amarillo",1,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Austin",2,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="El Paso",3,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Lubbock",4,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Dallas",5,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Fort Worth",6,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Plano",7,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Arlington",8,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Garland",9,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Irving",10,provider_numbers$X)
provider_numbers$X=ifelse(provider_numbers$City=="Grand Prairie",11,provider_numbers$X)




provider_numbers=subset(provider_numbers, X>0)

provider_numbers$X <- as.numeric(provider_numbers$X)
provider_numbers$t <- as.integer(provider_numbers$t)



#add column for value from each month for each city
df_combined <- provider_numbers %>%
  filter(t %in% c(0:7)) %>%
  dplyr::select(City, t, value) %>%
  pivot_wider(names_from = t, values_from = value, names_prefix = "value_") 
 

# Join back to original data frame

provider_numbers <- provider_numbers %>%
  left_join(df_combined, by = "City")


provider_numbers= as.data.frame(provider_numbers)

library('Synth')

# prep for synthetic control
dataprep.out <- dataprep(
  foo = provider_numbers,
  #predictors=value,
  predictors = c("value_0","value_2","value_3"),
  predictors.op = "mean",
  time.predictors.prior = c(0,2,3),
  dependent = "value",
  unit.variable = "X",
  unit.names.variable = "City",
  time.variable = "t",
  treatment.identifier = 2, 
  controls.identifier = c(11,1,3,4,5,6,7,8,9,10), 
  time.optimize.ssr = c((0):(4)),
  time.plot = c((0):(7)) 
)
...
path.plot(synth.res = synth.out, dataprep.res = dataprep.out, tr.intake=4,
          Ylab = "Number of Providers", Xlab = "Year")
...
gaps.plot(synth.res = synth.out, dataprep.res = dataprep.out,
          Ylab = "Gap in Number of Providers", Xlab = "Year", tr.intake = 4, Main = NULL)
...
exclude_times <- c(0,2,3)
# Define the pre-treatment period
pre_treatment_period <- 0:4
...
post_RMSPE = sqrt(mean(gaps[5:7]^2))
...
```{r}
#mean_duration is first calculated in the python file
#find the median of all the mean_duration variables
median_of_mean=median(subset(uber_providers,similar_to_austin==1 & is.na(mean_duration)==FALSE)$mean_duration)

#Define urban as providers who are more accessible via public transportation
...
#do this to find all providers who switch from more accessible to less accessible groups in the years of data
...
#in empirical analysis, filter out providers who switched groups
column1 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_filtered,similar_to_austin==1 &  switching==0))



column2 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + I(I(treatment_group*AfterReentry)*Urban) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_filtered,similar_to_austin==1 & switching==0))





stargazer(column1,column2, digits=2, no.space=TRUE, p.auto=TRUE, single.row=FALSE, omit.stat=c("ser", "adj.rsq","rsq"),
          add.lines = list(
                           c("Year fixed effects", 'Yes',"Yes"), 
                           c('Provider fixed effects','Yes','Yes'),
                           c('Other controls','Yes','Yes')), type="text")
...
traffic_data <- merge(traffic_data, census_df, by.y = c("year", "city"), by.x = c("year", "DIST_NM"),
  all.x = TRUE)
...
traffic_sdid=traffic_sdid[,c("ID","year","AADT","treated","standardized_pop")]
...
### Other Cities Analysis

```{r}
uber_providers$AfterReentry=ifelse(uber_providers$yearindicator>2017,1,0)


uber_providers$similar_to_rr=ifelse(uber_providers$Prscrbr_City=="Round Rock"|uber_providers$Prscrbr_City=="El Paso"|uber_providers$Prscrbr_City=="Amarillo"|uber_providers$Prscrbr_City=="Lubbock",1,0)


# Take out all providers who moved between treatment group and control cities
# First, get number of distinct cities for each provider within our subset

provider_city_count <- subset(uber_providers, similar_to_rr==1) %>%
  group_by(PRSCRBR_NPI) %>%
  summarise(city_count = n_distinct(Prscrbr_City))
...
# Check if these providers have Austin as one of their cities
providers_with_austin <- subset(uber_providers, similar_to_rr==1) %>%
  filter(Prscrbr_City == 'Round Rock') %>%
  pull(PRSCRBR_NPI) %>%
  unique()
...
uber_providers$similar_to_rr=ifelse(uber_providers$mixed==0 &(uber_providers$Prscrbr_City=="Round Rock"|uber_providers$Prscrbr_City=="El Paso"|uber_providers$Prscrbr_City=="Amarillo"|uber_providers$Prscrbr_City=="Lubbock"),1,0)

# Define treatment_group as any provider from Austin

temp=subset(uber_providers, Prscrbr_City=="Round Rock")
uber_providers$treatment_group=0
uber_providers[uber_providers$PRSCRBR_NPI %in% unique(temp$PRSCRBR_NPI),]$treatment_group=1


paralleltrends = felm(Tot_Clms ~ I(treatment_group*yearindicator) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome| PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers, similar_to_rr==1 & yearindicator<2018))
...
column4 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_rr==1))





stargazer(column4, digits=2, no.space=TRUE, p.auto=TRUE, single.row=FALSE, omit.stat=c("ser", "adj.rsq","rsq"),
          add.lines = list(
                           c("Year fixed effects", "Yes"), 
                           c('Provider fixed effects', 'Yes'),
                           c('Other controls', 'Yes')), type="text")

mean(subset(uber_providers,similar_to_rr==1 & treatment_group==0 )$Tot_Clms)
...
paralleltrends = felm(Tot_Clms ~ I(treatment_group*yearindicator)  + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome| PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(new_data_1, similar_to_austin==1 & yearindicator<2016 ))


stargazer(paralleltrends,  digits=2, no.space=TRUE, p.auto=TRUE, omit.stat=c("ser", "adj.rsq"),
          add.lines = list(
                           c("Year fixed effects", 'Yes','Yes'), 
                           c('Provider fixed effects', 'Yes','Yes')), type="text")
```
...
column3 = felm(Tot_Clms ~  I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome |PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(new_data_1,similar_to_austin==1))
...
paralleltrends = felm(Tot_Clms ~ I(treatment_laredo*yearindicator) +  Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|yearindicator + PRSCRBR_NPI|0|PRSCRBR_NPI, data=subset(uber_providers, yearindicator<2018 & similar_to_laredo==1))
...
column3 = felm(Tot_Clms ~ I(treatment_laredo*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_laredo==1))
...
column3 = felm(Tot_Clms ~ I(treatment_group*placeboAfter) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & yearindicator<2017))
...
merged_df <- merged_df %>%
  group_by(PRSCRBR_NPI) %>%
  mutate(standardized_elderlypop = ifelse(is.na(standardized_elderlypop), mean(standardized_elderlypop, na.rm = TRUE), standardized_elderlypop)) %>%
  ungroup()

merged_df <- merged_df %>%
  group_by(PRSCRBR_NPI) %>%
  mutate(standardized_pop = ifelse(is.na(standardized_pop), mean(standardized_pop, na.rm = TRUE), standardized_pop)) %>%
  ungroup()

merged_df <- merged_df %>%
  group_by(PRSCRBR_NPI) %>%
  mutate(standardized_medianincome = ifelse(is.na(standardized_medianincome), mean(standardized_medianincome, na.rm = TRUE), standardized_medianincome)) %>%
  ungroup()
...
column3 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator |0|PRSCRBR_NPI, data=subset(merged_df,similar_to_austin==1))
...
paralleltrends = felm(Tot_Clms ~ I(treatment_group*yearindicator) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome| PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers, similar_to_austin==1 & yearindicator<2016))

paralleltrends3 = felm(Tot_Benes ~ I(treatment_group*yearindicator) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome| PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers, similar_to_austin==1 & yearindicator<2016))
...
column1 = felm(Tot_Clms ~ I(treatment_group*PreExit) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator |0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & yearindicator<2018))

column2 = felm(Tot_Benes~ I(treatment_group*PreExit) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator |0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & yearindicator<2018))
...
paralleltrends = felm(Tot_Clms ~ I(treatment_group*yearindicator) +  Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|yearindicator + PRSCRBR_NPI|0|PRSCRBR_NPI, data=subset(uber_providers, yearindicator<2016 & similar_to_austin==1))
...
column3 = felm(Tot_Clms ~  I(treatment_group*PreExit) + I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 ))
...
column1 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + AfterReentry |PRSCRBR_NPI|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & yearindicator<2021))

column2 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) |PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & yearindicator<2021))


column3 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & yearindicator<2021))
...
stargazer(column1,column2,column3, digits=2, no.space=TRUE, p.auto=TRUE, omit.stat=c("ser", "adj.rsq","rsq"),
          add.lines = list(
                           c("Year fixed effects", 'No', 'Yes', 'Yes'), 
                           c('Provider fixed effects', 'Yes','Yes','Yes'),
                           c("Other controls", 'No', 'No', 'Yes')),type="latex")

mean(subset(uber_providers,similar_to_austin==1 & treatment_group==0 & yearindicator<2020)$Tot_Clms)
...
paralleltrends = felm(Tot_Clms ~ I(treatment_group*yearindicator) +  Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|yearindicator + PRSCRBR_NPI|0|PRSCRBR_NPI, data=subset(uber_providers, yearindicator<2018 & similar_to_austin==1 & moving_allyears==0))
...
column3 = felm(Tot_Clms ~ I(treatment_group*AfterReentry) + Bene_Avg_Risk_Scre + standardized_elderlypop + standardized_pop + standardized_medianincome|PRSCRBR_NPI + yearindicator|0|PRSCRBR_NPI, data=subset(uber_providers,similar_to_austin==1 & moving_allyears==0))
